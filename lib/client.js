...
.fischl-touch{position:fixed;left:50vw;top:0;width:50vw;height:100vh;pointer-events:auto;cursor:pointer;}
...
`;
    ...
    const LIB_ORDER = [
      { file: 'pixi.min.js', key: 'PIXI', cdn: 'https://cdn.jsdelivr.net/npm/pixi.js@6.5.10/dist/browser/pixi.min.js' },
      { file: 'live2dcubismcore.min.js', key: 'Live2DCubismCore', cdn: 'https://cubism.live2d.com/sdk-web/cubismcore/live2dcubismcore.min.js' },
      { file: 'cubism4.min.js', key: null, cdn: 'https://cdn.jsdelivr.net/npm/pixi-live2d-display@0.4.0/dist/cubism4.min.js' },
    ];
    let lineIdSeq = 1;
    function newLineId() {
      lineIdSeq += 1;
    ...
    function apply(ctx) {
      ...
      function applyTokens() {
        ...
        disposeThemeNow();
        try {
          themeDisposer = theme.register({ id: id, colorScheme: 'dark', tokens: tokens });
      ...
      let lastTokensKey = '';
      cfgListeners.add(() => {
        const key = JSON.stringify(buildTokens());
        if (key !== lastTokensKey) { lastTokensKey = key; applyTokens(); }
      });
      try { theme.setTheme('fischl'); } catch (error) { /* ignore */ }
      applyTokens();
      lastTokensKey = JSON.stringify(buildTokens());
      const store = {
      ...
      function ensureFischlTheme() {
        // 与 DEEP LIVE2D 页签一致的启用路径：动态注册新主题并切换（验证有效）
        if (cfg.theme.mode !== 'custom') cfg.theme.mode = 'preset';
        const key = JSON.stringify(buildTokens());
        if (key !== lastTokensKey) lastTokensKey = key;
        applyTokens();
      }
      let armed = true;
      // 用户点击「启用」后置位：DSH settings 同步（adopt）会把 preference 覆盖回官方值，
      // 只要 keepFischl 为真就持续抢回，保证菲谢尔主题真正保持
        ...
        if (!libsPromise) {
          libsPromise = (async () => {
            for (const lib of LIB_ORDER) {
              ...
              let ok = false;
              try {
                await Promise.race([loadScript('/fischl/fischl-libs/' + lib.file), ctx.timeout(20000).then(() => { throw new Error('timeout: ' + lib.file); })]);
                ok = true;
              } catch (error) {
                console.warn('fischl: local library unavailable, falling back to CDN: ' + lib.file);
              }
              if (ok && (!lib.key || w[lib.key])) continue;
              await Promise.race([loadScript(lib.cdn), ctx.timeout(30000).then(() => { throw new Error('CDN timeout: ' + lib.cdn); })]);
              if (lib.key && !w[lib.key]) throw new Error('library global missing: ' + lib.key);
            }
      ...
      function Live2DCanvas() {
        ...
        React.useEffect(() => {
          ...
          function positionAdjustElements() {
            ...
            if (!mode || !calibrated || measuredW <= 5) { adjustHandle.style.display = 'none'; adjustMark.style.display = 'none'; return; }
          ...
          const adjustBarBtns = {};
          for (const [m, label] of [['idle', '空闲位'], ['typing', '打字位'], ['head', '头部基准'], ['headTyping', '打字头部']]) {
          ...
          const pollTyping = () => {
            ...
            if (hasText && !store.typing) {
              ...
              publishStore();
              const touch = touchRef.current;
              if (touch) { touch.style.cursor = 'default'; try { touch.removeAttribute('title'); } catch (e) { /* ignore */ } }
              ...
              publishStore();
              const touch = touchRef.current;
              if (touch) { touch.style.cursor = 'pointer'; try { touch.setAttribute('title', '点击与皇女互动'); } catch (e) { /* ignore */ } }
            }
          };
        ...
        useStoreVersion();
        return React.createElement('div', { className: 'fischl-touch', ref: touchRef, onClick: onTouchClick, title: '点击与皇女互动' });
      }
