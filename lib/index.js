...
export function apply(ctx) {
  ...
  // ---- Cubism Core 固化 ----
  // 插件内置官方 Cubism Core（支持 moc3 v5）。注意 npm 的 live2dcubismcore@1.0.2
  // 是 2019 年 Cubism 4.0 版本：它能正常挂载全局对象，但无法解析 moc3 v5，
  // 模型会以 "Unknown error" 失败，因此唯一可信来源是官方 SDK Core。
  // 若文件缺失（克隆不完整、被 .gitignore 误排除等），host 会自动从官网补回。
  const CORE_REL = 'fischl-libs/live2dcubismcore.min.js';
  const CORE_URLS = [
  ...
  function ensureCore() {
    if (!corePromise) {
      corePromise = (async () => {
        const target = join(BUNDLE_MODEL_DIR, CORE_REL.replace(/\//g, '\\'));
        try {
          const info = await stat(target);
          if (info && info.size > 50000) return target;
        } catch (error) { /* 文件缺失 → 自动下载 */ }
        ...
        for (const url of CORE_URLS) {
          try {
            ...
            if (buffer.byteLength < 50000) continue;
            await mkdir(dirname(target), { recursive: true });
            await writeFile(target, buffer);
            console.log('fischl: Cubism Core 缺失，已自动补回 ' + buffer.byteLength + ' 字节 -> ' + target);
            return target;
          } catch (error) { /* 尝试下一个来源 */ }
        }
        console.warn('fischl: Cubism Core 缺失且自动下载失败，请手动放置：' + target);
        return null;
      })().catch(() => null);
    }
    return corePromise;
  ...
  const disposeRoute = webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        const dir = folderDirOf(folderKey, base);
        // 渲染库目录：core 缺失时先补回再返回（首次请求可能触发一次官网下载）
        if (folderKey === 'fischl-libs') await ensureCore();
